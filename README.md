...
6地点、固定道路、2台各4席、登録者5人・未予約者50人、9〜12時。需要予測は仮定値であり学習済みAIではない。Bの優位を固定値で埋め込まない。比較の集計は現在の地図とは独立した、予定変更なしの2回の全期間実行。
...
未予約者の位置・発生時刻・目的地はシードで再現。主要道路途中の1分刻みの位置に発生し、通過するバスが現地でのみ乗車を判定する。Bの優位は保証しない。
...
1. 右側の「9:40の実演へ（リセット）」を押す。
2. 公園→病院のまま「この移動を予約」を押す。
3. バス1の予定が「この時間帯の停留所到着予定なし」から「9:48 公園 → 9:54 駅」に変わる。灰色が予約前、車両色の破線が今後15分の予定、薄い実線が過去15分の実績。
4. 再生または時間スライダーで9:47の待機者、9:48の乗車、10:01の病院到着を確認する。

街並みは架空のイラスト。道路の接続・所要時間・距離モデルは従来のまま、描画だけを曲がりのある道路に変更。車両の移動とルート線は共通の道路形状上で補間する。

将来15分の予定は現在の状態のコピーから計算し、未確認の待機者・未来の未予約発生を与えない。これは確定運行の保証ではなく、新しい予約や実際の乗車によって変化する予定。プレビューによって本番状態や時刻は変化しない。

追加予約は3分後から乗車可能、12時を到着期限とする片道移動。登録者の集計分母にも追加する。A/B比較は追加予約・予定変更なしの元の5人で実行する。

追加検証: `node reservation.test.cjs`（予約の実乗降、視覚的ルート変更、プレビューへの未来需要漏洩防止）、`node city-map.test.cjs`（全道路の双方向端点と描画整合性）。

道路沿いの需要: `node roadside.test.cjs` で位置再現、道路上での乗車、遠隔待機情報を配車に使わないことを確認。細い生活道路は景観のみ、乗降停車時間は0分の簡易モデル。地域集計は最寄りの拠点を使用。

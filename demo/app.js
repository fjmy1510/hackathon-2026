...
$('compare').onclick=()=>{stop();const seed=Number($('seed').value)||42;const a=S.create(seed,'A'),b=S.create(seed,'B');for(let i=0;i<180;i++){S.step(a);S.step(b)}const am=S.metrics(a),bm=S.metrics(b),walkinTotal=a.people.filter(p=>p.kind==='walkin').length;const c=$('comparison');c.hidden=false;c.innerHTML=`<div class="panel-head"><b>同じ条件、2つの運行。</b><span class="tag">シード ${seed} / 追加予約・予定変更なし / 12:00</span></div><table><thead><tr><th>評価指標</th><th>A 予定のみ</th><th>B ＋需要予測</th></tr></thead><tbody><tr><td>登録者の予定達成率</td><td>${am.registered*20}%（${am.registered}/5人）</td><td>${bm.registered*20}%（${bm.registered}/5人）</td></tr><tr><td>未予約者の移動達成率</td><td>${Math.round(am.walkin/walkinTotal*100)}%（${am.walkin}/${walkinTotal}人）</td><td>${Math.round(bm.walkin/walkinTotal*100)}%（${bm.walkin}/${walkinTotal}人）</td></tr><tr><td>総走行距離</td><td>${Number(am.distance).toFixed(1)} km</td><td>${Number(bm.distance).toFixed(1)} km</td></tr>${S.nodes.map(n=>`<tr><td>${esc(n.name)} 発の未予約・未達人数</td><td>${am.unmet[n.name]??am.unmet[n.id]??0}人</td><td>${bm.unmet[n.name]??bm.unmet[n.id]??0}人</td></tr>`).join('')}</tbody></table><p class="note">B−A：未予約の到着 ${bm.walkin-am.walkin>=0?'+':''}${bm.walkin-am.walkin}人 / 走行距離 ${(bm.distance-am.distance).toFixed(1)} km。現在の地図は比較実行前の状態を保持しています。未達人数には終了時点の乗車中・待機中・未発生を含みます。</p>`;c.scrollIntoView({behavior:'smooth',block:'start'})};
